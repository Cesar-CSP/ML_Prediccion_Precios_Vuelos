...
En la validación cruzada del modelo XGBoost sin optimizar se obutuvo un score medio F1 de 0.63 aproximadamente. Tras la optimización de hiperparámetros usando la librería Optuna, se mejora a un 0.7. Después de entrenar el modelo, al evaluar el conjunto de test obtenemos un F1-macro de 0.71 aproximadamente, por lo tanto el modelo gereraliza bien y no se observa overfitting ni underfitting. En cuanto a la matriz de confusión, el modelo muestra un sesgo positivo hacia la detección de la clase Business, caracterizándose por un Recall excepcionalmente alto (~98%): de los vuelos reales Business, el modelo logra capturar 18,357 frente a solo 278 omisiones. Sin embargo, esta sensibilidad penaliza la Precisión de Business (~52%), ya que el modelo genera un volumen considerable de falsos positivos (vuelos Economy clasificados erróneamente como Business). En contrapartida, para la clase Economy, el modelo es altamente fiable (alta precisión) cuando predice dicha clase (~99%), pero presenta un Recall moderado, permitiendo que un número significativo de vuelos Economy "escapen" hacia la categoría Business.
